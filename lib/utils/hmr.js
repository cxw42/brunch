...
exports.generate = (generateModuleFiles, preModuleFiles, postModuleFiles, processor, deppack, definition, path, root) => {
  const hmrPath = sysPath.join('hmr-brunch', 'runtime.js');
  const hmrFile = preModuleFiles.find(f => f.path.includes(hmrPath)) ||
    postModuleFiles.find(f => f.path.includes(hmrPath));
  ...
  preModuleFiles.filter(f => f !== hmrFile).forEach(processor);
  ...
  postModuleFiles.filter(f => f !== hmrFile).forEach(processor);
