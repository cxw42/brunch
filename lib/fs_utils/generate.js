...
function concat(files, path, definitionFn, autoRequire, config) {
  ...
  if (isJs) {
    ...
    debug(`moduleFiles: ${moduleFiles.map(f=>f.path).join(', ')}`);
    debug(`nonModuleFiles: ${nonModuleFiles.map(f=>f.path).join(', ')}`);

    // Split the non-module files between those before the modules and those
    // after the modules.
    const postWrapped = (config && config.files && config.files.javascripts &&
        config.files.javascripts.postWrapped) || [];
    const postModuleFiles =
      nonModuleFiles.filter(f => anymatch(postWrapped, f.path));
    const preModuleFiles =
      nonModuleFiles.filter(f => !postModuleFiles.includes(f));

    debug(`preModuleFiles: ${preModuleFiles.map(f=>f.path).join(', ')}`);
    debug(`postModuleFiles: ${postModuleFiles.map(f=>f.path).join(', ')}`);
    ...
    function basicGenerate(generateModuleFiles, preModuleFiles, postModuleFiles, processor, deppack, definition, path, root) {
      root.add(definition);
      preModuleFiles.forEach(processor);
      generateModuleFiles();
      postModuleFiles.forEach(processor);
    };

    const generator = isHmr ? hmr.generate : basicGenerate;
    generator(generateModuleFiles, preModuleFiles, postModuleFiles, processor, deppack, definition, path, root);
